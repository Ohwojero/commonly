...
const galleryImages = (product: Product) => [
  product.image,
  product.image.replace('w=1000', 'w=800').replace('q=85', 'q=85') + '&grayscale',
  product.image.replace('w=1000', 'w=800') + '&crop=top',
  product.image.replace('w=1000', 'w=800') + '&crop=bottom',
]
    ...
    <div className="min-h-screen bg-background pt-28">
      <div className="mx-auto grid max-w-[1280px] gap-8 px-4 pb-6 pt-6 sm:px-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,.85fr)] lg:gap-10 lg:px-8 lg:pb-10 lg:pt-10">
        <section className="grid min-w-0 grid-cols-[56px_minmax(0,1fr)] gap-4 lg:grid-cols-[64px_minmax(0,1fr)]">
          <div className="flex flex-col gap-3">
            {images.map((image, index) => (
              <button
                ...
                className={`aspect-square overflow-hidden rounded-xl border-2 bg-muted transition-all duration-200 ${activeImage === index ? 'border-primary ring-2 ring-primary/20' : 'border-transparent hover:border-border'}`}
              >
          ...
          >
            <img src={images[activeImage]} alt={product.name} className="size-full object-cover transition-all duration-300" />
            {isZooming && (
              <div
                aria-hidden="true"
